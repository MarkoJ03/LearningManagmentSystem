...
public class TerminNastaveService extends BaseService<TerminNastave, TerminNastaveDTO, Long>{
	...
	protected TerminNastaveDTO convertToDTO(TerminNastave entity) {
		...
		KalendarDTO kalendar = new KalendarDTO(entity.getKalendar().getId(), null);
		
		return new TerminNastaveDTO(entity.getId(), entity.getVremePocetka(), entity.getVremeKraja(), entity.getBrojCasova(), realizacijaPredmeta, kalendar);
	}
	...
	protected TerminNastave convertToEntity(TerminNastaveDTO dto) {
		RealizacijaPredmeta realizacijaPredmeta = new RealizacijaPredmeta(dto.getRealizacijaPredmeta().getId(),
				null, null, null, null ,null);
		Kalendar kalendar = new Kalendar(dto.getKalendar().getId(), null);
		
		return new TerminNastave(dto.getId(), dto.getVremePocetka(), dto.getVremeKraja(), dto.getBrojCasova(), realizacijaPredmeta, kalendar);
	}
